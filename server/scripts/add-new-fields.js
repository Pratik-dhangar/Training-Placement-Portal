...
async function addNewFields() {
  try {
    ...
    console.log('Adding new fields to personal_details table...');
    
    // Check if github column exists
    const githubExists = await pool.query(`
      SELECT EXISTS (
    ...
    if (!githubExists.rows[0].exists) {
      await pool.query(`
        ALTER TABLE personal_details 
        ADD COLUMN github TEXT;
      `);
      console.log('Added github column to personal_details table');
    } else {
      console.log('github column already exists in personal_details table');
    }

    // Check if social_media column exists
    const socialMediaExists = await pool.query(`
      SELECT EXISTS (
        SELECT FROM information_schema.columns 
        WHERE table_name = 'personal_details' 
    ...
    if (!socialMediaExists.rows[0].exists) {
      await pool.query(`
        ALTER TABLE personal_details 
        ADD COLUMN social_media TEXT;
      `);
      console.log('Added social_media column to personal_details table');
    } else {
      console.log('social_media column already exists in personal_details table');
    }

    console.log('Adding new fields to academic_details table...');
